...
public interface IProyectoAdjuntosUseCase
{
    Task<IReadOnlyList<ProyectoAdjuntoDto>> ListAsync(Guid proyectoId, CancellationToken cancellationToken = default);
    Task<ProyectoAdjuntoDto> CrearAsync(Guid proyectoId, CrearProyectoAdjuntoDto input, CancellationToken cancellationToken = default);
    ...
    Task<ProyectoAdjuntoDto> SubirAsync(Guid proyectoId, string nombreArchivo, string contentType, long tamanoBytes, Stream contenido, CancellationToken cancellationToken = default);
    ...
    Task<string> ObtenerUrlDescargaAsync(Guid proyectoId, Guid id, CancellationToken cancellationToken = default);

    Task EliminarAsync(Guid proyectoId, Guid id, CancellationToken cancellationToken = default);
}
...
/// y enlaces" lista para Proyectos desde antes, pero apuntaba a un endpoint que todavía no existía acá.
/// </summary>
public class ProyectoAdjuntosUseCase(IProyectoRepository proyectos, IProyectoAdjuntoRepository adjuntos, ISupabaseStorageService storage, IUnitOfWork unitOfWork) : IProyectoAdjuntosUseCase
{
    ...
    public async Task<IReadOnlyList<ProyectoAdjuntoDto>> ListAsync(Guid proyectoId, CancellationToken ct = default)
    {
        await AsegurarProyecto(proyectoId, ct);
        return (await adjuntos.GetByProyectoIdAsync(proyectoId, ct)).Select(Map).ToList();
    }

    public async Task<ProyectoAdjuntoDto> CrearAsync(Guid proyectoId, CrearProyectoAdjuntoDto input, CancellationToken ct = default)
    {
        await AsegurarProyecto(proyectoId, ct); Validar(input);
        var adjunto = new ProyectoAdjunto { ProyectoId = proyectoId, Tipo = input.Tipo, Nombre = input.Nombre.Trim(), Url = input.Url?.Trim(), StoragePath = input.StoragePath?.Trim(), Meta = input.Meta?.Trim(), Fecha = input.Fecha ?? DateTime.UtcNow };
        await adjuntos.AddAsync(adjunto, ct); await unitOfWork.SaveChangesAsync(ct);
        return Map(adjunto);
    }

    public async Task<ProyectoAdjuntoDto> SubirAsync(Guid proyectoId, string nombreArchivo, string contentType, long tamanoBytes, Stream contenido, CancellationToken ct = default)
    {
        await AsegurarProyecto(proyectoId, ct);
        ...
        var storagePath = $"proyectos/{proyectoId}/{Guid.NewGuid()}-{nombreSaneado}{extension.ToLowerInvariant()}";

        await storage.SubirAsync(storagePath, contenido, contentTypeEsperado, ct);

        var adjunto = new ProyectoAdjunto
        {
    ...
    public async Task<string> ObtenerUrlDescargaAsync(Guid proyectoId, Guid id, CancellationToken ct = default)
    {
        await AsegurarProyecto(proyectoId, ct);
        var adjunto = await adjuntos.GetByIdAsync(id, ct);
        if (adjunto is null || adjunto.ProyectoId != proyectoId) throw new EntityNotFoundException("ProyectoAdjunto", id);
    ...
    public async Task EliminarAsync(Guid proyectoId, Guid id, CancellationToken ct = default)
    {
        await AsegurarProyecto(proyectoId, ct);
        var adjunto = await adjuntos.GetByIdAsync(id, ct);
        if (adjunto is null || adjunto.ProyectoId != proyectoId) throw new EntityNotFoundException("ProyectoAdjunto", id);
    ...
    private async Task AsegurarProyecto(Guid id, CancellationToken ct) { if (await proyectos.GetByIdAsync(id, ct) is null) throw new EntityNotFoundException("Proyecto", id); }
    private static void Validar(CrearProyectoAdjuntoDto input)
    {
    ...
    private static ProyectoAdjuntoDto Map(ProyectoAdjunto x) => new() { Id = x.Id, ProyectoId = x.ProyectoId, Tipo = x.Tipo, Nombre = x.Nombre, Url = x.Url, StoragePath = x.StoragePath, Meta = x.Meta, ContentType = x.ContentType, TamanoBytes = x.TamanoBytes, Fecha = x.Fecha, CreatedAt = x.CreatedAt };
}
